...
Everything downstream of `model.py` imports it as `from MA_vax import model` (or
`counterfactual`, `fit_bayesian`, etc.), so run all commands below from the repo
root with `uv run python MA_vax/<script>.py ...`.
...
```
uv run marimo edit MA_vax/ma_vax.py
```
...
```
uv run python MA_vax/fit_bayesian.py --method both            # emcee + pyabc, with m(t)
uv run python MA_vax/fit_bayesian.py --method emcee --no-tvbeta
```
...
**Outputs:** a timestamped `MA_vax/outputs_<date>_<time>h<min>m<sec>s/` folder
...
```
uv run python MA_vax/run_scenarios.py --fit-folder MA_vax/outputs_2026-07-08_15h24m30s
uv run python MA_vax/run_scenarios.py --fit-folder MA_vax/outputs_2026-07-08_15h24m30s \
...
uv run python MA_vax/run_counterfactual_tables.py \
    --fit-folder MA_vax/outputs_2026-07-08_15h24m30s --method emcee --n-reps 500
uv run python MA_vax/run_counterfactual_tables.py --deterministic   # fast sanity check
```
...
**Outputs:** a timestamped `MA_vax/counterfactual_tables_<date>_<time>h<min>m<sec>s/`
...
```
uv run marimo edit MA_vax/counterfactual_notebook.py
```
...
`MA_vax/` code reads from `MA_vax/` itself:
