...
When fitting the Massachusetts flu+vaccination model (`model_config_MA.json` /
...
(`MA_vax/fit_bayesian.py` + `MA_vax/model.py`). This document records the
...
exact hazard formula `1 - exp(-rate * dt)`; `MA_vax/model.py` uses the linear
...
- **`humidity_impact` prior bounds.** The reference fit (`MA_vax/fit_bayesian.py`)
...
1. `MA_vax/model.py`'s `simulate_new_H` — pure numpy, one Euler step per day
   (`dt=1`, i.e. effectively `ts_per_day=1`).
2. `generic_core`'s `ConfigDrivenMetapopModel`, built from
   `model_config_MA.json`, run at `ts_per_day=1` (matched to `model.py`, so
...
- **`MA_vax/model.py` (linear / forward-Euler):** `p = rate · dt`
...
- `MA_vax/model.py`'s linear scheme isn't a bug either — it's simply *what the
...
All in `generic_core/examples/massachusetts_vax/` unless noted:

- **`MA_vax/compare_to_generic.py`** — apples-to-apples forward-simulation
  comparison between `MA_vax/model.py` and `generic_core`, for a given fitted
  parameter set (§1.2). Also writes `MA_vax/compare_to_generic_trajectories.csv`
...
- **`peak_matched_ts7_vs_ts224.csv`** — the peak-matched shape comparison in
  ...
  `ts_per_day=7` at the beta adjusted to match its peak).
